...
const string Data::NOMS_CURTS_DIES_SETMANA[] = {"dl", "dt", "dc", "dj", "dv", "ds", "dg"};
...
//------------Constructors------------
Data::Data() : any(2000), mes(1), dia(1) 
{

}

Data::Data(int any, int mes, int dia) : any(any), mes(mes), dia(dia) 
{
    if (not es_data_valida(any, mes, dia)) {
        this->any = 2000;
        this->mes = 1;
        this->dia = 1;
...
Data::Data(int any, string mes, int dia) : any(any), mes(0), dia(dia) 
{
    ...
    if (not es_data_valida(any, this->mes, dia)) {
        this->any = 2000;
        this->mes = 1;
        this->dia = 1;
    }
...
Data::Data(const Data& d) :  any(d.any), mes(d.mes), dia(d.dia)
{
...
bool Data::es_data_valida(int any, int mes, int dia) 
{
    return  any >= ANY_INICI_CALENDARI
        and mes >= 1 and mes <= MESOS_ANY
        and dia >= 1 and dia <= dies_mes(any,mes);
}
...
void Data::escriure_calendari(int any){
    for(int mes = 1; mes <= MESOS_ANY; mes++) {
        escriure_calendari(any, mes);
    }
}

void Data::escriure_calendari(int any, int mes)
{
    //Ratlla separadora    
    ...
    int nombre_dies_mes = dies_mes(any, mes);
...
int Data::dies_mes(int any, int mes) 
{
